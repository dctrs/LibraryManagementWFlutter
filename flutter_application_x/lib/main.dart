...
class _BottomNavBarState extends State<BottomNavBar> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Osman Tosun Kütüphane Yönetimi'),
        actions: <Widget>[
          IconButton(
...
class _BookFormPageState extends State<BookFormPage> {
  ...
  void _addOrUpdateBook() async {
    ...
    if (widget.document == null) {
      ...
      Fluttertoast.showToast(msg: 'Kitap başarıyla eklendi.');
    } else {
      await firestore.collection('books').doc(widget.document!.id).update(bookData);
      Fluttertoast.showToast(msg: 'Kitap başarıyla güncellendi.');
    }
